...
def sistEuler(deltat,t,x,y,omega):
    for i in range(100):
        t += deltat
        x += deltat*y
        y += deltat*(-omega**2*x)
...
def sistRK2(deltat,t,x,y,omega):
    for i in range(100):
        t += deltat
        k1x = deltat*y
        k1y = deltat*(-omega**2*x)
        k2x = deltat*(y+k1y/2.)
        k2y = deltat*(-omega**2*(x+k1x/2.))
        x += k2x
        y += k2y
        plt.plot(t,x,'*k')
        
def sistRK4(deltat,t,x,y,omega):
    for i in range(100):
        t += deltat
        k1x = deltat*y
        k1y = deltat*(-omega**2*x)
        ...
        k2y = deltat*(-omega**2*(x+k1x/2.))
        k3x = deltat*(y+k2y/2.)
        k3y = deltat*(-omega**2*(x+k2x/2.))
        k4x = deltat*(y+k3y)
        k4y = deltat*(-omega**2*(x+k3x))
...
sistEuler(deltat,t,x,y,omega)
sistRK2(deltat,t,x,y,omega)
sistRK4(deltat,t,x,y,omega)
